...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if not self.use_gt_subg:
            self.subgraph_mask = HybridLoader("data/{}_graph_mask_{}_rm_duplicate".format(dataset_name, mask_version),
                                              ".npz")
        else:
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        tmp_list = self._prefetch_process[
            split].get()  # call one time to get a whole batch instead of fetching one by one instance
        wrapped = tmp_list[-1]
        tmp_list = tmp_list[:-1]
    ...
    def __getitem__(self, index):
        ...
        if not self.use_gt_subg:
            ...
            for i in range(self.seq_per_img):
                ...
                if pos_idx.shape[0] < self.half_mini_batch:  # pos not enough, pad with sentence noun subgraph
                    ...
                    mask_idx[i, :to_pad, 0] = i - 5;
                    mask_idx[i, to_pad:, 0] = pos_idx
                else:
            ...
            gpn_pool_mtx = np.zeros((self.seq_per_img, 2, self.half_mini_batch, self.obj_num, self.obj_num)).astype(
                'float32')
            for i in range(self.seq_per_img):
        ...
        pad_object_fmap = np.full((1, self.obj_num, object_fmap.shape[1]), 0).astype(
            'float32')  # pad with the dummy/empty node
        pad_object_dist = np.concatenate(
            (np.ones((1, self.obj_num, 1)), np.zeros((1, self.obj_num, object_dist.shape[1] - 1))), axis=2).astype(
            'float32')
        ...
        pad_rel_ind = np.full((1, self.rel_num, rel_ind.shape[1]),
                              self.obj_num - 1)  # pad the rel_ind with the dummy/empty node index
        pad_pred_dist = np.concatenate(
            (np.ones((1, self.rel_num, 1)), np.zeros((1, self.rel_num, pred_dist.shape[1] - 1))), axis=2).astype(
            'float32')
...
class BlobFetcher():
    ...
    def reset(self):
        ...
        self.split_loader = iter(data.DataLoader(dataset=self.dataloader,
                                                 ...
                                                 # should same as the number in ri_next = ri + self.batch_size
